...
- [X] T068 将 `web/src/components/`, `web/src/features/`, `web/src/views/` 中的产品自有英文文案直接迁移到 `web/src/locales/zh-CN.ts` 和领域化中文资源，移除对 `web/src/composables/useUiLocalization.ts` 挂载后文本改写的依赖，并将 `scripts/check-ui-localization.sh` 升级为覆盖文本节点、属性、运行时消息和精确技术术语豁免的全量门禁 per FR-001/FR-002/FR-024/SC-001 and plan: centralized text (contradicts)
