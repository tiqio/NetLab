...
func (r *TopologyRepository) FinalizeLaboratoryDeletion(ctx context.Context, id domain.ID) error {
	return r.database.Write(ctx, func(tx *sql.Tx) error {
		statements := []string{
			...
			`DELETE FROM topology_endpoint_reservations WHERE laboratory_id=?`,
			`DELETE FROM operation_tasks WHERE resource_id IN (SELECT id FROM network_object_links WHERE laboratory_id=?)`,
			`DELETE FROM operation_tasks WHERE resource_id IN (SELECT id FROM network_attachments WHERE network_object_id IN (SELECT id FROM network_objects WHERE laboratory_id=?))`,
			`DELETE FROM operation_tasks WHERE resource_id IN (SELECT id FROM links WHERE laboratory_id=?)`,
			`DELETE FROM operation_tasks WHERE resource_type='topology_connection' AND json_extract(input_json,'$.laboratory_id')=?`,
			`DELETE FROM network_object_links WHERE laboratory_id=?`,
