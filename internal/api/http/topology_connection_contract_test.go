...
func (s *topologyConnectionCommandsStub) Create(ctx context.Context, laboratoryID domain.ID, source, target domain.ConnectionEndpoint, _ domain.TopologyConnectionConfig, _ string) (domain.TopologyConnection, domain.OperationTask, error) {
...
func TestUnifiedTopologyConnectionCreateContract(t *testing.T) {
	...
	if commands.entryPoint != "port_drag" {
