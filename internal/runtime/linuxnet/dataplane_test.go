...
func TestNamespaceAttachmentRepairsL2VLANDriftWhenHostPathIsHealthy(t *testing.T) {
	attachment := domain.NetworkAttachment{ID: "attach", InterfaceID: "if-a", PortName: "lan0", Config: map[string]any{"pvid": 20, "tagged": []any{30}}}
	...
	for _, expected := range []string{
		"bridge -n " + namespace + " vlan del dev lan0 vid 1-4094",
		"bridge -n " + namespace + " vlan add dev lan0 vid 20 pvid untagged",
		"bridge -n " + namespace + " vlan add dev lan0 vid 30",
	} {
		if !strings.Contains(commands, expected) {
			t.Fatalf("missing VLAN repair %q in %s", expected, commands)
		}
	}
}
