...
function connection(id: string, source: string, target: string): ConnectionPresentation {
  return {
    id,
    persistedKind: "node_link",
    source: { resourceId: source, resourceType: "node", resourceKind: "docker", resourceName: source, portId: `${source}-0`, portName: "eth0", endpointKey: `${source}/${source}-0` },
    target: { resourceId: target, resourceType: "node", resourceKind: "docker", resourceName: target, portId: `${target}-0`, portName: "eth0", endpointKey: `${target}/${target}-0` },
    desiredState: "connected",
    actualState: "connected",
    statusVisual: { state: "connected", label: "已连接", colorToken: "var(--topology-connection-success)", lineType: "solid", width: 2, cue: "normal" },
    semanticMarkers: [],
    routeGroupKey: "",
    routeIndex: 0,
    routeCount: 1,
...
describe("assignParallelRoutes", () => {
  it("groups every connection kind by unordered resource pair", () => {
    const values = [connection("b", "right", "left"), connection("a", "left", "right"), connection("c", "left", "right")];
