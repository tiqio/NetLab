...
describe("TopologyConnectionLegend", () => {
  ...
  it.each(["light", "dark"])("supports collapse, scrolling, and accessible names in %s theme", async (theme) => {
    document.documentElement.setAttribute("data-theme", theme);
    const wrapper = mount(TopologyConnectionLegend, {
      props: {
        items: [
          { key: "managed-nat-uplink", label: "NAT 管理上联", description: "NAT 描述", count: 1, connectionIds: ["nat"] },
          { key: "shared-broadcast-domain", label: "共享广播域", description: "广播域描述", count: 3, connectionIds: ["a", "b", "c"] },
        ],
      },
    });
    ...
    expect(legend.get('button[aria-expanded="false"]').attributes("aria-label")).toContain("展开");
  });
});
