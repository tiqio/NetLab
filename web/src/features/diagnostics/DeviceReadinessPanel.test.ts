...
const { getDeviceReadiness } = vi.hoisted(() => ({
...
describe("DeviceReadinessPanel", () => {
  ...
  it("separates cable, guest, management and data path states", async () => {
    getDeviceReadiness.mockResolvedValue({
      node_id: "node-1",
      roles: [],
      cable: { state: "ready" },
      guest: { state: "ready" },
      management: { state: "prerequisite" },
      data_path: { state: "unverified" },
    });
    const wrapper = mount(DeviceReadinessPanel, {
      props: {
        node: {
          id: "node-1",
          name: "vendor",
